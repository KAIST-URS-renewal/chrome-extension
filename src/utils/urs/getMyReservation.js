...
const parseMyReservationHtml = res => {
  ...
  const myreservation_list = root.querySelectorAll('.table_list > tbody > tr');
  ...
  _.forEach(myreservation_list, value => {
    ...
    const checkinDate = _.trim(_.split(reservationCheckinDatetime, '\n')[0]);
    ...
    const checkoutDate = _.trim(_.split(reservationCheckoutDatetime, '\n')[0]);
    var checkoutTime = _.trim(_.split(reservationCheckoutDatetime, '\n')[1]);
    ...
    const deadlineDate = _.trim(_.split(reservationCancelDeadline, '\n')[0]);
