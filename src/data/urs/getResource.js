...
const parseResourceHtml = res => {
  if (!res || res.includes('시스템 오류') || res.includes('Error')) {
  ...
  _.forEach(resource_list, value => {
    ...
    // get resource id
    const resourceId = value.querySelector('td > input').getAttribute('value');
    ...
    if (_.isEqual(resourceLocation, 'N/A') | _.isEqual(resourceLocation, '')) {
    ...
    if (_.isEqual(resourceCapacity, 'N/A') | _.isEqual(resourceCapacity, '')) {
...
export const getResource = async facilityJsonArray => {
  ...
  const resourceInfo = _.flatten(
    _.map(htmls, html => {
      // if html is null, then return false
